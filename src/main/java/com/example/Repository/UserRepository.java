...
public interface UserRepository extends JpaRepository<User, Integer>
{
  ...
  @Query(value = "select u.*,r.roles from user as u inner join role as r on r.id = u.role_id where r.id != 1 and r.id != 2 and (u.firstname LIKE CONCAT('%',?1,'%') or u.lastname LIKE CONCAT('%',?1,'%'))", nativeQuery = true)
