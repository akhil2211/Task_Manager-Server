...
public interface TaskHistoryRepo extends CrudRepository<TaskHistory,Integer> {
    @Query(value = "select u.username, th.unassigned_at,th.assigned_at from user as u join task_history as th on u.id=th.previoususer_id where task_id=?",nativeQuery = true)
